...
def bert_score(df_pred, mp):
    ...
    for i, ind in tqdm(enumerate(df_pred.index), total=len(df_pred)):
        ...
        pre, rec, f1 = score([counters], [ref_list], lang='hi', rescale_with_baseline=True)
        bs_p+=pre
        bs_r+=rec
        bs_f1+=f1
        #print(pre, rec, f1)
...
def bert_score_main(df_train, df_test, df_pred):    
...
#         df_test = pd.read_csv(test_path, lineterminator='\n')
    ...
    bs_p, bs_r, bs_f1 = bert_score(df_pred, mp)
