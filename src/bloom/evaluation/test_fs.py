...
class Counterspeech_generate():
    ...
    def __init__(self, path, index):
        ...
        self.train_data = pd.read_csv(parser['paths']['base_path']+parser['paths']['data_train'],encoding='utf-8',lineterminator='\n')
        self.test_data = pd.read_csv(parser['paths']['base_path']+parser['paths']['data_test'],encoding='utf-8',lineterminator='\n')
        #self.pred_data = pd.read_csv(parser['paths']['base_path']+parser['paths']['data_pred'],encoding='utf-8',lineterminator='\n')
        ...
        self.model = torch.load(parser['paths']['base_path']+parser['models']['saved_model']+'_'+str(index)+'.pt',map_location=self.dev)
        print("Model laoded : ", parser['paths']['base_path']+parser['models']['saved_model']+'_'+str(index)+'.pt')
        self.tokenizer = BloomTokenizerFast.from_pretrained(self.model_name)
        
        self.output_path_json = parser['paths']['base_path']+parser['outputs']['output_path']+'_'+str(index)+'.json'
        self.output_path_csv = parser['paths']['base_path']+parser['outputs']['output_path']+'_'+str(index)+'.csv'
        self.output_path_scores = parser['paths']['base_path']+parser['outputs']['output_path']+'_'+str(index)+'.txt'
        self.batch_size = int(parser['models']['batch_size'])
        self.language = parser['models']['language']
    ...
    def generate_mt5(self):
        ...
        for i in tqdm(list(range(num_data)), desc="Generating Counter: "):
            result = {}
            input_ids = self.tokenizer.encode('counterspeech: '+preprocess(self.test_data.iloc[i]['input_text'])+'</s>',return_tensors="pt")
    ...
    def generate_gpt2(self):
        ...
        for i in tqdm(list(range(num_data)), desc="Generating Counter: "):
            result = {}
            input_ids = self.tokenizer.encode(preprocess(self.test_data.iloc[i]['input_text'])+'</s><s>',return_tensors="pt")
            input_ids=input_ids.to(self.dev)
            with torch.no_grad():
                output = self.model.generate(input_ids=input_ids, 
                                         pad_token_id         = self.tokenizer.eos_token_id,
    ...
    def get_scores(self):
        ...
        bs_p, bs_r, bs_f1 = bert_score_main(self.train_data, self.test_data, self.pred_data, self.language)
        f.write("\n\n\nBERT Scores")
