...
#     print(avg_novelty(df_pred['predicted_text'], df_test['target_text']), avg_novelty(df_pred['predicted_text'], df_train['target_text']))
    ...
    bleu = bleu_1 = bleu_2 = bleu_3 = bleu_4 = meteor_ = 0.0
    ...
    for ind in df_pred.index:
        ...
        bleu_1  += nltk.translate.bleu_score.sentence_bleu(ref_list, word_tokenize(counters),
                                                           smoothing_function=SmoothingFunction().method2, weights=(1.0, 0, 0, 0))
        ...
        bleu_3  += nltk.translate.bleu_score.sentence_bleu(ref_list, word_tokenize(counters), 
                                                           smoothing_function=SmoothingFunction().method2, weights=(0.33, 0.33, 0.33, 0))
        bleu_4  += nltk.translate.bleu_score.sentence_bleu(ref_list, word_tokenize(counters), 
                                                           smoothing_function=SmoothingFunction().method2, weights=(0.25, 0.25, 0.25, 0.25))
